"""Build Phase 2 training data v3 — combines both formats.

1. MemRL format (system + few-shot + task) — for MemRL evaluation
2. Raw format (just task description) — for general use

Both weighted by Q-values.

Usage:
    python scripts/build_training_data_v3.py results/memory_bank.json results/training_v3.jsonl
"""
...
def build_training_data(memory_bank_path, alfworld_data_dir, few_shot_path, output_path):
    ...
    for mem in memories:
        ...
        # FORMAT 1: MemRL format (system + few-shot + task)
        ...
        # FORMAT 2: Raw format (just task description → full plan)
        raw_messages = [
    ...
    print(f"Total training examples (Q-weighted, both formats): {len(training_data)}")
...
if __name__ == "__main__":
    if len(sys.argv) != 5:
        print("Usage: python scripts/build_training_data_v3.py <memory_bank.json> <alfworld_data_dir> <few_shot.json> <output.jsonl>")
        sys.exit(1)
